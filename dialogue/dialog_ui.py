...
class DialogueUI:
    def __init__(self, name, full_text, portrait, side, font):
        ...
        self.portrait = portrait  # Objeto Surface ya escalado desde la escena
        self.side = side 
        self.font = font
        
        # --- SISTEMA DE ANIMACIÓN ---
        self.anim_progress = 0.0 
        self.anim_speed = 7.0   # Un poco más suave
    ...
    def _anim_slide(self, rect, progress):
        """Sube con un pequeño rebote visual"""
        offset = 40 * (1.0 - progress)
        rect.y += offset
        return rect
    ...
    def draw(self, screen, y_pos, current_text, fixed_x, is_last=False):
        ...
        if self.anim_progress > 0.1:
            ...
            # Dibujar Nombre (Etiqueta superior)
            if self.name and self.anim_progress > 0.5:
                ...
                if self.side == 'left':
                    name_rect.topleft = (draw_rect.x + 15, draw_rect.y - 12)
                else:
                    name_rect.topright = (draw_rect.right - 15, draw_rect.y - 12)
                
                # Fondo pequeño para el nombre
    ...
    def _draw_content(self, screen, rect, current_text, is_last=False):
        ...
        # Gestión de Retrato (Opcional en la burbuja, ya que están los grandes arriba)
        if self.portrait:
            # Re-escalamos solo si es necesario para la burbuja (miniatura)
            p_img = pygame.transform.smoothscale(self.portrait, (80, 80))
            text_margin = 95
            if self.side == 'right':
                p_x = rect.right - 80 - self.padding_x
                text_x = rect.x + self.padding_x
            else:
                p_x = rect.x + self.padding_x
                text_x = p_x + text_margin
            
            # Dibujar círculo de fondo para el portrait miniatura
            p_rect = pygame.Rect(p_x, draw_y + (rect.height - 80) // 2, 80, 80)
            pygame.draw.ellipse(screen, (230, 230, 230), p_rect)
            screen.blit(p_img, p_rect.topleft)
        else:
            text_x = rect.x + self.padding_x

        # Ajuste de texto envolvente
        ...
        if is_last and current_text.strip() == self.full_text.strip():
            ...
            if time_blink > 400: # Efecto parpadeo
                small_font = pygame.font.SysFont("sans-serif", 14, bold=True)
                hint_surf = small_font.render("▼ ESPACIO", True, (120, 120, 120))
                h_x = rect.right - hint_surf.get_width() - 15
                h_y = rect.bottom - hint_surf.get_height() - 8
                screen.blit(hint_surf, (h_x, h_y))
    ...
    def get_expected_height(self):
        ...
        return max(content_h, 85) # Altura mínima consistente